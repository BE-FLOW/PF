...
테스터에게는 `https://pf-two-eta.vercel.app`만 전달한다. 테스트 계정은 이메일과
비밀번호로 가입하며, 서비스 안내와 테스트 연락을 위한 `010` 휴대전화번호를
필수로 입력한다. 가입 확인 메일은 현재 테스트 프로젝트에서 생략한다.
...
- GitHub Actions `CI`가 통과한 변경만 테스트 주소에서 확인한다.
- DB 변경은 새 migration 파일로 추가한다.
- 비밀키와 실제 테스트 이메일은 커밋하지 않는다.
- 배포 전 lint, test, build를 모두 통과한다.
