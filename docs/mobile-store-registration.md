...
- 패치 반영 커밋 `c20c98b`에서 새 production 후보를 병렬 요청했다.
  ...
  `98932a10-b24c-4dca-8d14-2be8ca2a5e66`다. 이후 두 빌드 모두 완료됐다.
  Android는 Play에 아직 업로드하지 않았고 iOS 배포 결과는 아래에 기록했다.
  최종 스크린샷과 실기기 QA도 이 새 후보에 맞춰야 하며 build `32/34`의 승인을
  그대로 이전하지 않는다.
- 기존 iOS 스크린샷은 build `22`이므로 무료 공개용으로 제출하지 않는다.
  ...
  `33343738614`이며 인증 사전검증과 자격증명 입력 이후 스크린샷 `0/5`로 실패했다.
  후속 로컬 커밋 `caff4a19beeb09f7bfe70a19eef2adc49b5c88b6`은 로그인과 데이터
  준비 확인 단계를 분리했지만 아직 push·재실행하지 않았다. iOS 즉시 설치를
  우선해 캡처 후속 작업은 보류했다.

### iOS 무료 외부 베타 배포 완료

사용자의 최신 우선순위는 iOS 사용자가 빠르게 내려받아 확인하는 것이다.
App Store 스크린샷과 Android 후속 작업을 기다리지 않고 기존 무료 production
build `35`를 TestFlight 외부 테스트로 배포했다. 새 빌드는 추가로 만들지 않았다.

- EAS build `f9dfedc8-e262-4892-aa27-877af4852190`와 submission
  `24829c7c-5bbc-4ca6-b8fd-17a0d0a97419`의 빌드·업로드가 완료됐다.
  소스는 `c20c98bed85004920d821d4725b6f19d60c185a8`이다.
- ASC build `b5d9ba79-1d53-43e0-a873-5abdf2bb9b0b`는 `VALID`,
  `expired: false`, 최소 iOS `16.4`다. 베타 심사는 `APPROVED`이며 내부·외부
  배포 상태 모두 `IN_BETA_TESTING`인 것을 API 재조회로 확인했다.
- `PetFlow 보호자 테스트` 그룹 `331e0971-a75a-456f-bb02-dfe40a675f07`의
  공개 링크는 활성 상태이며 별도 참여 인원 제한은 설정하지 않았다.
  [펫플로우 무료 베타 참여](https://testflight.apple.com/join/vMJkQKMt)
- 외부 그룹에는 build `35`만 남겼다. 과거 build
  `28/21/19/17/16/15/14/13/6`은 그룹 연결만 해제했다. 빌드 자체를 삭제하거나
  만료시키지 않았으므로 필요한 경우 그룹에 다시 연결할 수 있다.
- 브라우저에서 공개 링크의 제목 `펫플로우 베타에 참여하기 - TestFlight - Apple`과
  TestFlight 설치 후 기기에서 링크를 열라는 안내를 확인했다.
- 설치 안내: iPhone에 TestFlight 설치 → 위 공개 링크 열기 → 수락 → 펫플로우 설치.
  기존 테스터는 업데이트한다. 실제 iPhone 설치·사용 성공을 대신 검증한 것은 아니다.

이 기록은 **무료 TestFlight 베타 배포 완료**다. App Store 정식 공개 심사·출시 완료와
...
- 캡처 브랜치의 `caff4a1`과 이미 완성된 Simulator `36`을 재사용해 GitHub Actions
  ...
  이 실행은 인증 사전검사와 자격증명 입력 후 `verify-login` 단계에서 실패했다.
  첫 실행 안내 모달을 먼저 닫도록 `8568e5d`에서 순서를 고쳤지만 재실행
  `33363387482`도 같은 단계에서 실패했다. 두 실행 모두 스크린샷 산출물이 없으며
  실제 앱 로그인 실패로 확정한 결과는 아니다. 보호 로그는 삭제되고 Simulator도
  정리됐다. 다음 진단은 키보드 닫기·스크롤·탭·화면 대기 중 실패 명령을 구분하는
  고정 코드만 남기며, 민감 로그를 공개해서 원인을 찾지 않는다.
  마지막 최소 수정 `b9499f8`은 `hideKeyboard`만 제거하고 로그인·모달 검증은
  유지했다. 실패 유형 고정 코드와 UI 고정 문구 8개의 존재 여부만 출력한다.
  문법·민감값 비출력·무결성 검사를 거쳐 `33364184887`을 시작했으며, 결과는
  아직 확인 전이다. 추가 재시도 전에 이 실행 결과와 실패 코드를 먼저 읽는다.
...
- 버전 상태는 `PREPARE_FOR_SUBMISSION`, 출시 방식은 `AFTER_APPROVAL`이다. 심사 제출은
  하지 않았다.
- App Store 가격표는 수동 1개와 자동 174개 지역 모두 `0`으로 재확인했다. build `39`는
  비면제 암호화 미사용, 아이콘 포함, 최소 iOS `16.4`이며, 앱은 `4+`·라이프스타일
  카테고리로 설정돼 있다. 과거 일회성 인앱상품은 앱 레코드에 남아 있지만 이번 버전에
  연결하거나 심사 대상으로 제출하지 않는다.
...
- 과거 build `22` manifest가 새 후보를 암묵적으로 선택하지 않도록 readiness 명령은
  ...
  manifest를 만들지 않는다.
- App Store Connect 웹 재확인에서는 Apple 로그인이 만료돼 개인정보 응답과 배포 지역을
  열지 못했다. 사용자 재로그인 후 두 화면을 최종 확인해야 하며 법적·금융 정보나 2단계
  인증값은 자동 입력하거나 추측하지 않는다.
...
- 건강 관련 사용자 콘텐츠: 관찰 기록, 예방접종·월별 예방관리 완료,
  보호자가 옮긴 검사 결과와 병원 안내
...
5. 서로 다른 예방접종을 연속 추가해 기존 기록이 덮어써지지 않는지 확인한다.
6. 월별 예방관리 완료·취소와 과거 기록 삭제, 검사 기록 저장·수정·삭제·공유를 확인한다.
7. 새 계정으로 병원 준비부터 AI 전달본 생성·기기 공유까지 3분 안에 마친다.
8. AI 초안이 진단이나 수의사 확인 정보처럼 보이지 않는지 표본 검수한다.
9. 공정사용 한도와 초기화 시점이 명확하며 기본 사실 전달본은 계속 제공되는지 확인한다.
10. 앱 전체를 순회해 상품·가격·구매·복원·남은 이용권·참여코드가 없는지 확인한다.
11. 기기와 프록시 로그에서 RevenueCat 초기화·상품 조회·결제 동기화가 없는지 확인한다.
12. 계정 탈퇴 뒤 Auth, DB 행과 Storage 파일 접근이 차단되는지 확인한다.
