...
현재 모바일 빌드는 로그인, 반려동물 등록·수정·삭제·선택, 짧은 건강
기록 입력, 기본 안전 분류, 최근 기록과 건강 흐름, 사진·동영상 첨부, 기록 수정·삭제,
자동 사건 연결, 병원 공유 기본 요약, 보호자가 옮긴 병원 안내, 병원 전달본,
병원 전달본 피드백, 즉시 계정 탈퇴까지 구현되어 있다. 별도 경과 입력이나 흐름 마무리를
...
- 연령 등급: 9+
- 가격: 무료
- 개인정보 답변: 이메일 주소, 사용자 ID, 사진 또는 비디오,
...
- App Privacy Details에서 이메일, 사용자 ID, 건강 관련 사용자 입력,
  사진·동영상, 구매 내역, 제품 상호작용 사용 여부를 실제 빌드 기준으로 신고한다.
- 병원 전달본은 AI 정리이며 수의사 확인 정보가 아닌 초안이라고 심사 메모와 앱 UI에 표시한다.
...
- 첫 병원 전달본 1회는 무료로 제공한다.
- 이후 `병원 전달본 1회 이용권`을 Apple·Google 인앱결제로 판매한다.
